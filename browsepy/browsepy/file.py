...
class File(Node):
    ...
    * :attr:`generic` is set to False, so static method :meth:`from_urlpath`
    ...
    def blockchain_status(self):
        return adapter.getBlockchainStatus(self.path)

    @cached_property
    def verification_status(self):
        return adapter.getVerificationStatus(self.path)

    @cached_property
    def confirmation_date(self):
        return adapter.getConfirmationDate(self.path)

    @cached_property
    def transaction_id(self):
        return adapter.getTransactionId(self.path)

    @cached_property
    def is_tracked(self):
        return adapter.isTracked(self.path)

    @cached_property
    def verified_hash(self):
        return adapter.verifiedHash(self.path)

    @cached_property
    def calculated_hash(self):
        return adapter.calculateHash(self.path)
